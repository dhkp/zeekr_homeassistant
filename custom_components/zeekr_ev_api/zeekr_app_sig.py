...
def _maybe_hex_to_bytes(s: str) -> bytes:
    """Accept either hex-encoded strings or raw ASCII strings.

    Heuristic: if the string is all hex chars AND decoding from hex
    yields a valid AES key/IV length (16, 24, or 32 bytes), treat as hex.
    Otherwise keep as raw UTF-8 bytes (handles EU-style 16-char ASCII keys
    like 'a01a6db985a2f5d4' which look like hex but ARE the key bytes).
    """
    s = s.strip()
    utf8_bytes = s.encode("utf-8")
    if re.fullmatch(r"[0-9a-fA-F]+", s) and len(s) % 2 == 0:
        hex_bytes = bytes.fromhex(s)
        # If UTF-8 length is already a valid AES size but hex length is not,
        # prefer UTF-8 (e.g., 16-char hex string = 16 bytes UTF-8 vs 8 bytes hex)
        valid_aes_sizes = {16, 24, 32}
        if len(utf8_bytes) in valid_aes_sizes and len(hex_bytes) not in valid_aes_sizes:
            return utf8_bytes
        return hex_bytes
    return utf8_bytes
...
def aes_encrypt(plain_text: str, key_hex: str, iv_hex: str) -> str:
    """
    Encrypts a string using AES/CBC/PKCS7 (PKCS5-compatible) and returns Base64 ciphertext.

    NOTE:
    - Despite the historical parameter names, `key_hex` / `iv_hex` may be either:
      - hex-encoded bytes (recommended), OR
      - raw strings (fallback)
    - AES key lengths supported by PyCryptodome: 16/24/32 bytes.
    - IV must be 16 bytes.
    """
    plain_text_bytes = plain_text.encode("utf-8")

    key_bytes = _maybe_hex_to_bytes(key_hex)
    iv_bytes = _maybe_hex_to_bytes(iv_hex)
