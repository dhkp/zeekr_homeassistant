...
class ZeekrClient:
    ...
    def __init__(
        ...
        # Headers used for HMAC-signed (gateway) requests
        self.hmac_headers = (hmac_headers.copy() if hmac_headers else const.DEFAULT_HEADERS.copy())
        # Best-effort: keep country fields consistent
        if country_code:
            self.hmac_headers["country"] = country_code
            self.hmac_headers["registcountry"] = country_code
